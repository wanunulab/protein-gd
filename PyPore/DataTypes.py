...
class Event( Segment ):
    ...
    def plot( self, hmm=None, cmap="Set1", algorithm='viterbi', color_cycle=['r', 'b', '#FF6600', 'g'],
        ...
        plt.xlabel( "Time (s)" )
        plt.ylabel( "Current (nA)" )#changed from (pA)
...
class File( Segment ):
    ...
    def __init__( self, filename=None, current=None, timestep=None, **kwargs ):
        ...
        if current is not None and timestep is not None:
            ...
            timestep, current =  load_bdat( filename ) #read_abf( filename ) 
            filename = filename.split("\\")[-1].split(".bdat")[0]
        else:
    ...
    def plot( self, limits=None, color_events=True, event_downsample=5, 
        ...
        limits = limits or ( 0, len( self.current) / second)
        start, end = limits
        ...
        #########################################################################################################
            ...
            print("end: ",end)
            current = self.current[ int(start*second):int(end*second):downsample ]
            plt.plot( np.arange( 0, len(current) )*step*downsample+start, current, **file_kwargs)#**kwargs )
...
class Experiment( object ):
    ...
    def parse( self, event_detector=lambda_event_parser( threshold=90 ), 
        ...
        for file in map( File, self.filenames ):
            if verbose:
